...
class IQFITApp(App):
    ...
    def build(self):
        ...
        with main_window.canvas:
            self.rect = Rectangle(
                source='./images/background.png', size=main_window.size)

        logo = Image(source='./images/logo.png', size_hint=(0.1, 0.1),
                     pos_hint={'center_y': 0.9, 'center_x': 0.1})
        main_window.add_widget(logo)

        title_label = Label(text="IQFIT solver", font_size='30sp', bold=True, valign='middle',
                            size_hint=(.5, .25), pos_hint={'center_y': 0.95, 'center_x': 0.5})
        main_window.add_widget(title_label)

        instructions_label = Label(text="Select pieces and place them on the board.\nPress R to rotate pieces (in 2D and 3D)\nWhen done press Solve to see solution.",
                                   font_size='20sp', bold=True, halign='left', valign='middle', size_hint=(.5, .25), pos_hint={'center_y': 0.75, 'center_x': 0.3})
        main_window.add_widget(instructions_label)

        iqfit_pieces = Piece.iqfit_pieces()

        pieces_grid = PiecesGrid(pieces=iqfit_pieces, cols=2, rows=len(
            iqfit_pieces)//2, size_hint=(.3, .7), pos_hint={'center_y': 0.4, 'center_x': 0.8})
        self.pieces_grid = pieces_grid
        main_window.add_widget(pieces_grid)

        board_grid = BoardGrid(pieces=pieces_grid, cols=10, rows=5, spacing=2,
                               size_hint=(.55, .55), pos_hint={'center_y': 0.4, 'center_x': 0.325})
        self.board_grid = board_grid
        main_window.add_widget(board_grid)

        clear_button = Button(text="Clear board", size_hint=(.15, .08), pos_hint={
                              'center_y': 0.05, 'center_x': 0.5}, background_color="red")
        clear_button.bind(on_press=self.clear_board)
        main_window.add_widget(clear_button)

        solve_button = Button(text="Solve board", size_hint=(.15, .08), pos_hint={
                              'center_y': 0.05, 'center_x': 0.3}, background_color="green")
        ...
        for board_button in self.board_grid.grid:
            ...
            if selected_piece is not None:
                ...
                if selected_piece not in init_pieces:
                    ...
                    init_pieces[selected_piece] = (min(init_pieces[selected_piece][0], row), min(
                        init_pieces[selected_piece][1], col))
...
class PieceButton(ToggleButton):
    def __init__(self, piece: Piece, **kwargs):
        ...
        self.piece = [Piece(piece.name, coords)
                      for coords in piece.get_transformations()]
...
class PiecesGrid(GridLayout):
    ...
    def __init__(self, pieces, **kwargs):
        ...
        for piece in pieces:
            piece_button = PieceButton(piece=pieces[piece], group="iqfit_pieces",
                                       background_normal=f"./images/iqfit-pieces/{piece}.png", background_down=f"./images/iqfit-pieces/{piece}")
...
class BoardGrid(GridLayout):
    def __init__(self, pieces: PiecesGrid, **kwargs):
        ...
        for i in range(self.rows*self.cols):
            ...
            self.grid[board_button] = {
                "id": i, "hovered": False, "border_point ": None, "selected": None}
    ...
    def on_enter(self, board_button):
        ...
        if self.pieces.selected is None:
            ...
            selected_coords = self.pieces.iqfit_pieces[selected_name].get_transformations_2D()[
                self.rotation]
            selected_grid_coords = set(self.grid[board_button]["id"]+x+self.cols*y if col +
                                       x <= self.cols-1 else self.rows*self.cols+1 for x, y in selected_coords)
